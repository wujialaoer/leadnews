        ...
        <div class="channel-menu border-bottom">
          <div class="affix">
            <slider class="switcher-wrapper" ref="slider">
              <switcher :list="regionTags" displayType="start" :indexTab="indexTab"
                        @switchTab="changeContent"></switcher>
            </slider>
          ...
          <transition name="sliderTab">
            <div class="drawer" v-show="toggleTab" @click="toggleList">
              <ul @click.stop>
                <li v-for="(item, index) in regionTags" :key="index" class="tab-item"
                    :class="{ active: index === indexTab }" @click="changeContent(index)">
                  <span>{{ item.name }}</span>
                </li>
